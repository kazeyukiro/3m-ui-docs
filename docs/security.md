...
7. 在 **系统设置 → 安全** 中可开启管理员 TOTP（兼容 Google Authenticator、微软验证器等）。
8. 流程：点击设置 → 用验证器扫描二维码或录入密钥 → 输入动态码启用。
9. 启用后，登录需先校验账号密码，再输入 6 位验证码；可在同页关闭（需密码与验证码）。
10. 丢失验证器且无法登录时，需在服务器上通过数据库或运维手段关闭 TOTP（勿在公网暴露调试接口）。

## 面板路径前缀（web_path）

11. 可在配置中设置 `server.web_path`（例如 `/secret-panel`），使管理界面与 API 挂在自定义前缀下，降低扫描面。
12. 修改后需重启面板；浏览器访问 `https://域名/secret-panel/`。
13. 公开订阅路径默认仍可按原规则访问（不依赖管理员前缀），具体以当前版本路由为准。

## 暴露面

14. 面板端口默认不对公网裸奔。Docker 部署默认绑定 `127.0.0.1:8080`，需通过反向代理（nginx/Caddy）暴露。
15. 仅开放 443 到反代；面板端口不对公网开放。
16. HTTPS：面板与订阅全程 TLS。支持 Let's Encrypt 自动证书（ACME）或手动证书。

## 订阅与 Token

17. 订阅 Token 等同于密码；用户离职即删号或轮换 token。
18. Access Token（192 位随机）有过期时间，绑定到特定 Listener。
19. Token 泄露后立即禁用或删除。
20. 可为用户设置 **HWID 设备上限**，限制能导入订阅的终端数量（见 [用户与流量](/users-traffic)）。
